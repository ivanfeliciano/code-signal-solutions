...
class Trie(object):
	...
	def print_elements(self):
		...
		while len(cola) > 0:
			...
			flag = False
			# if len(cola_aux) > 0:
			# 	print("{} children: ".format(cola_aux.pop(0)))
			for i in range(len(top.references)):
				if top.references[i] != None:
					flag = True
					cola.append(top.references[i])
					print(string.ascii_letters[i], end="\t")
					cola_aux.append(string.ascii_letters[i])
			if flag: print()
...
def findSubstrings(words, parts):
	...
	for word in words:
		...
		while i < len(word) and start_pos < len(word):
			if word == "drake":
				print(word[start_pos: i + 1])
			is_key, part = mi_trie.is_in_the_trie(word[start_pos: i + 1])
			if not is_key:
				if word == "drake":
					print("{} is not a key".format(word[start_pos: i + 1]))
				start_pos += 1
				i = start_pos
				continue
			if is_key and part != -1 and len(best_part) < len(part):
				if word == "drake":
					print("Candidate for best part {} at idx {}".format(part, start_pos))
				best_idx = start_pos
				# start_pos += 1
				best_part = part
			i += 1
...
if __name__ == '__main__':
	...
	# print('\n'.join(findSubstrings(words, parts)))
